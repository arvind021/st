...
async def cmd_done(client: Client, message: Message):
    ...
    # custom_batch session takes priority if active
    from plugins.mod_features import custom_batch_sessions
    if user_id in custom_batch_sessions:
        return  # handled by mod_features.py group=10
...
async def cmd_cancel(_, message: Message):
    ...
    elif user_id in custom_batch_sessions:
        count = len(custom_batch_sessions.pop(user_id))
        await message.reply(f"❌ Custom batch cancelled. ({count} file(s) discarded)")
    else:
        await message.reply("No active batch session.")
